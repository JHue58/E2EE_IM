...
import com.tencent.qcloud.tuikit.tuichat.Encrypt.EncryptConfig;
...
public class ChatMessageParser {
    ...
    public static TUIMessageBean parseMessage(V2TIMMessage v2TIMMessage, boolean isIgnoreReply) {
        ...
        if (!isIgnoreReply){
            //解密消息(用RSA+AES实现端对端加密)By Nana
            EncryptConfig encryptConfig = new EncryptConfig(v2TIMMessage);
            encryptConfig.deEncrypt();
            EncryptUtil.encryptMessage(encryptConfig);
        }
    ...
    public static TUIMessageBean parseEncryptMessage(V2TIMMessage v2TIMMessage){
        V2TIMCustomElem customElem = v2TIMMessage.getCustomElem();
        return null;
    }
    ...
    public static String getLocalImagePath(TUIMessageBean messageBean) {
        ...
        if (file.exists()) {
            return EncryptUtil.imageDeEncryptToGetPath(path);
        }
        return null;
    }
